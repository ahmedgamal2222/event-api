...
speakersRouter.post('/', requireAdmin, async (c) => {
  try {
    ...
    console.error('POST /speakers error:', error);
    return c.json({ success: false, error: error?.message || 'Server error' }, 500);
  }
});
...
speakersRouter.put('/:id', requireAdmin, async (c) => {
  try {
    ...
    console.error('PUT /speakers/:id error:', error);
    return c.json({ success: false, error: error?.message || 'Server error' }, 500);
  }
});
...
speakersRouter.delete('/:id', requireAdmin, async (c) => {
  try {
    const id = Number(c.req.param('id'));
    const eventId = Number(c.req.param('eventId'));
    const result = await c.env.DB.prepare('DELETE FROM speakers WHERE id = ? AND event_id = ?').bind(id, eventId).run();
    
    if (!result.meta.changes) {
      return c.json({ success: false, error: 'Speaker not found' }, 404);
    }
    
    return c.json({ success: true });
  } catch (error: any) {
    console.error('DELETE /speakers/:id error:', error);
    return c.json({ success: false, error: error?.message || 'Server error' }, 500);
  }
});
