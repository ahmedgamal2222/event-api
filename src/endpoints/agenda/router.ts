...
agendaRouter.post('/days', requireAdmin, async (c) => {
  try {
    ...
    console.error('POST /agenda/days error:', error);
    return c.json({ success: false, error: error?.message || 'Server error' }, 500);
  }
});
...
agendaRouter.put('/days/:id', requireAdmin, async (c) => {
  try {
    ...
    // Check if day exists
    const existing = await c.env.DB.prepare('SELECT id FROM agenda_days WHERE id = ?').bind(id).first();
    if (!existing) return c.json({ success: false, error: 'Agenda day not found' }, 404);
    ...
    console.error('PUT /agenda/days/:id error:', error);
    return c.json({ success: false, error: error?.message || 'Server error' }, 500);
  }
});
...
agendaRouter.post('/sessions', requireAdmin, async (c) => {
  try {
    ...
    console.error('POST /agenda/sessions error:', error);
    return c.json({ success: false, error: error?.message || 'Server error' }, 500);
  }
});
...
agendaRouter.put('/sessions/:id', requireAdmin, async (c) => {
  try {
    ...
    const existing = await c.env.DB.prepare('SELECT id FROM agenda_sessions WHERE id = ? AND event_id = ?').bind(id, eventId).first();
    ...
    console.error('PUT /agenda/sessions/:id error:', error);
    return c.json({ success: false, error: error?.message || 'Server error' }, 500);
  }
});
...
agendaRouter.delete('/sessions/:id', requireAdmin, async (c) => {
  try {
    const id = Number(c.req.param('id'));
    const eventId = Number(c.req.param('eventId'));
    const result = await c.env.DB.prepare('DELETE FROM agenda_sessions WHERE id = ? AND event_id = ?').bind(id, eventId).run();
    ...
    console.error('DELETE /agenda/sessions/:id error:', error);
    return c.json({ success: false, error: error?.message || 'Server error' }, 500);
  }
});
